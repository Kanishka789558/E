...
//     if (prods) setProducts((prev) => [...prev, ...prods]);
...
//             </div>
//             <Image src={`/${p.image ?? "placeholder.png"}`} alt={p.name} width={200} height={180} />
//             <h3>{p.name}</h3>
//             <div className={styles.price}>
//               <span className={styles.newPrice}>${p.price.toFixed(2)}</span>
//             </div>
...
//   const scrollRef = useRef<HTMLDivElement>(null);
//   const limit = 4; // 👈 Control products per page
...
//     // ✅ Limit + offset correctly applied
//     const { data: prods } = await supabase
//       .from("products")
//       .select("id,name,price,image")
//       .order("id", { ascending: true })
...
//     setOffset(nextOffset);
//     loadProducts(nextOffset); // ✅ Now fetches next batch
//   };
